...
def load_station_level_cache():
    master_path = CACHE_DIR / "master_station_table.csv"
    ts_path = CACHE_DIR / "timeseries_flow.csv"
    od_path = CACHE_DIR / "od_agg.csv"

    if not (master_path.exists() and ts_path.exists() and od_path.exists()):
        raise FileNotFoundError("站点级缓存文件不存在，请先运行预处理脚本。")

    master = safe_read_csv(master_path, encoding="utf-8", low_memory=False)
    ts = safe_read_csv(ts_path, encoding="utf-8", low_memory=False)
    od = safe_read_csv(od_path, encoding="utf-8", low_memory=False)
    return master, ts, od


def build_station_level_cache_from_raw():
    return clean_raw_od()
...
def get_or_build():
    ...
    if master_path.exists() and ts_path.exists() and od_path.exists():
        return load_station_level_cache()

    return build_station_level_cache_from_raw()